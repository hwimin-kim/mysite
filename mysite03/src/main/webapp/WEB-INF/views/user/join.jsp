...
$(function(){
	$("#join-form").submit(function(event){
		...
		//1. 이름 유효성(empty) 체크
		if($("#name").val() === '') {
			messageBox("회원가입", "이름이 비어 있습니다.", $("#name").focus());
			return;
		}
		//2. 이메일 유효성(empty) 체크
		if($("#email").val() === '') {
			messageBox("회원가입", "이메일이 비어 있습니다.", $("#email").focus());
			return;
		}
		...
		//4. 비밀번호 유효성(empty) 체크	
		if($("#password").val() === '') {
			messageBox("회원가입", "비밀번호가 비어 있습니다.", $("#password").focus());
			return;
		}
		...
		$.ajax({
			...
			success: function(response) {
				if(response.result !== 'success') {
					cosnole.error(response.message);
					return;
				}
				
				if(response.data) {			// exists!
						messageBox("회원가입", "존재하는 이메일 입니다. 다른 이메일을 사용하세요", $('#email').focus());
						$('#email')
							.val('')
							.focus();
						return;
		...
		<div id="content">
			<div id="user">
				...
				<form:form
					...
					<p style="test-align:left; padding:0; color:red">
